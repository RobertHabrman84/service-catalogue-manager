#!/usr/bin/env pwsh
# ============================================================================
# Service Catalogue Manager - START ALL (FIXED V4 - TRUE NO EF CORE)
# ============================================================================
# Version: 4.3.0
# Description: Starts DB with db_structure.sql ONLY - TRULY NO EF Core migrations
...
$DB_SETUP_SCRIPT = Join-Path $SCRIPT_DIR "database\scripts\setup-db-fixed-v3.ps1"
$DB_STRUCTURE_SQL = Join-Path $SCRIPT_DIR "database\schema\db_structure.sql"
...
function Show-Help {
    Write-Header "SERVICE CATALOGUE MANAGER - START SCRIPT v4.3.0 (TRUE NO EF CORE)"
    
    Write-Host "USAGE:" -ForegroundColor $COLOR_INFO
    Write-Host "  .\start-all-fixed-v4.ps1 [OPTIONS]"
    Write-Host ""
    ...
    Write-Host "  .\start-all-fixed-v4.ps1 -UseDocker -RecreateDb"
    Write-Host ""
    Write-Host "  # Pouze databáze s kompletní strukturou"
    Write-Host "  .\start-all-fixed-v4.ps1 -UseDocker -RecreateDb -DbOnly"
    Write-Host ""
    Write-Host "  # SQLite pro sandbox režim"
    Write-Host "  .\start-all-fixed-v4.ps1 -UseSQLite -RecreateDb"
    Write-Host ""
...
function Start-Database {
    Write-Header "STARTING DATABASE (TRUE NO EF CORE - PURE SQL)"
...
function Setup-DockerDatabase {
    ...
    # Použít výhradně setup-db-fixed-v3.ps1 - žádné EF Core migrace!
    $setupScript = Join-Path $SCRIPT_DIR "database\scripts\setup-db-fixed-v3.ps1"
    if (Test-Path $setupScript) {
        ...
        Write-ErrorMessage "Database CANNOT be initialized without setup-db-fixed-v3.ps1"
...
function Verify-DatabaseStructure {
    ...
    try {
        ...
        if (-not [string]::IsNullOrEmpty($numericLine)) {
            $tableCount = [int]::Parse($numericLine)
        } else {
            # Alternativní přístup - najít první číslo v jakémkoli řádku
...
function Test-Prerequisites {
    Write-Header "CHECKING PREREQUISITES (TRUE NO EF CORE VERIFICATION)"
...
function Start-Frontend {
    ...
    try {
        ...
        # OPRAVA: Použití správné cesty k npm
        $npmPath = Resolve-Executable -Command "npm" -Fallbacks @("npm.cmd", "npx.cmd", "npx")
        if (-not $npmPath) {
        ...
        # OPRAVA: Použití správných argumentů pro npm
        $arguments = @("run", "dev", "--", "--port", $FRONTEND_PORT)
        
        # OPRAVA: Použití Start-Process s přímým voláním npm přes cmd.exe
        # Toto je nejspolehlivější způsob jak spustit npm na Windows
        $frontendProcess = Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", "npm run dev -- --port $FRONTEND_PORT") -WorkingDirectory $FRONTEND_DIR -PassThru -WindowStyle Normal
...
function Start-All {
    Write-Header "STARTING ALL SERVICES (TRUE NO EF CORE MODE)"
...
# Show startup header
Write-Header "SERVICE CATALOGUE MANAGER - STARTUP v4.3.0 (TRUE NO EF CORE)"
Write-Info "POUŽÍVÁ VÝHRADNĚ db_structure.sql - ŽÁDNÉ EF CORE MIGRACE!"
Write-Info "Verze: 4.3.0 - Kompletní odstranění EF Core migrací - OPRAVENÁ VERZE"
Write-Host ""
