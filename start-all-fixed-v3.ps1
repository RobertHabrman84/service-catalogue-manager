#!/usr/bin/env pwsh
# ============================================================================
# Service Catalogue Manager - START ALL (FIXED V3 - TRUE NO EF CORE)
# ============================================================================
# Version: 4.2.0
...
function Show-Help {
    Write-Header "SERVICE CATALOGUE MANAGER - START SCRIPT v4.2.0 (TRUE NO EF CORE)"
    
    Write-Host "USAGE:" -ForegroundColor $COLOR_INFO
    Write-Host "  .\start-all-fixed-v3.ps1 [OPTIONS]"
    Write-Host ""
    ...
    Write-Host "  .\start-all-fixed-v3.ps1 -UseDocker -RecreateDb"
    Write-Host ""
    Write-Host "  # Pouze databáze s kompletní strukturou"
    Write-Host "  .\start-all-fixed-v3.ps1 -UseDocker -RecreateDb -DbOnly"
    Write-Host ""
    Write-Host "  # SQLite pro sandbox režim"
    Write-Host "  .\start-all-fixed-v3.ps1 -UseSQLite -RecreateDb"
    Write-Host ""
...
function Start-Frontend {
    ...
    try {
        ...
        # OPRAVA: Použití Start-Process s FilePath a ArgumentList
        $frontendProcess = Start-Process -FilePath "cmd.exe" -ArgumentList @("/c", "npm", "run", "dev", "--", "--port", $FRONTEND_PORT) -WorkingDirectory $FRONTEND_DIR -PassThru
...
# Show startup header
Write-Header "SERVICE CATALOGUE MANAGER - STARTUP v4.2.0 (TRUE NO EF CORE)"
Write-Info "POUŽÍVÁ VÝHRADNĚ db_structure.sql - ŽÁDNÉ EF CORE MIGRACE!"
Write-Info "Verze: 4.2.0 - Kompletní odstranění EF Core migrací - OPRAVENÁ VERZE"
Write-Host ""
